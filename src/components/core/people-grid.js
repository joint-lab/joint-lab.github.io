...
function AlumniThumbnail({ firstName, lastName, role, nextRole, slug }) {
  return (
    <li className="mb-4">
      <Link to={slug}>
        <div className="text-lg leading-6 font-medium">
          <h3 className="group-hover:underline">{firstName} {lastName}</h3>
          <p className="text-gray-600 text-sm">First moved to: {nextRole}</p>
          <p className="text-green-600 text-sm">Graduated as: {role}</p>
        </div>
      </Link>
    </li>
  );
...
export function AlumniGrid({ title, people, className }) {
  const halfLength = Math.ceil(people.length / 2);
  const leftColumn = people.slice(0, halfLength);
  const rightColumn = people.slice(halfLength);
    ...
    <div className={className}>
      <Container>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
          <ul>
            {leftColumn.map(({ node }, index) => (
              <AlumniThumbnail key={index} {...node.frontmatter} {...node.fields} />
            ))}
          </ul>
          <ul>
